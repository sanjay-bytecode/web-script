	...
	//$postid = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE post_title LIKE '".$fund_name."'");
		...
		function fundpost_filter_by_loaction()
		{
			...
			<div class="row campaign-list post-listing">
				...
				{
					echo "<div class='alert alert-danger fs-error-fund fs-error-fund-zip'><div class='row campaign-list'><span style='color:red;'>There are no campaigns available.</span></div></div>";
				}
				?>
						...
						while ($query11->have_posts()) : $query11->the_post(); 
							...
							<tr>
								<td>
									...
									{
										?>
										<div class="partner-radio">
											<input type="radio" name="get_value" value="<?php echo the_title();?>" id="<?php echo $postid = get_the_ID();?>" data-id=<?php echo $postid = get_the_ID();?>>
											<label for="<?php echo $postid = get_the_ID();?>"></label>
										</div>
										<?php } else { ?>
										<div class="partner-radio">
											<input id="dis-able" type="radio" name="get_value" disabled />
											<label for="dis-able"></label>
										</div>
										<?php } ?>
									</td>
									<td>
                ...
                // send the email to reatiler
							...
							function search_reat_post()
							{
								...
								if(isset($_POST['search_ret_namess']) && !empty($_POST['search_ret_namess']))
								{
									...
									$args4 = null;
									$args4 = array( 'post_type' => 'retailer', 's' =>$ret_names, 'posts_per_page' => 10,'post_status' =>'publish','paged' => $paged4,'author' =>  $user_ret_ids);
...
//include("load-more-scroll-functions.php");